...
<div class="container">
    ...
    <div class="room-details">
        <h2>King Suite</h2>
        <p><strong>Størrelse:</strong> 100 m²</p>
...
// Function to get a random available room ID
function getRandomAvailableRoomId($start_date, $end_date, $conn, $roomType = 'King Suite') {
    $query = "
        SELECT r.id
        FROM rooms r
...
$assignedRoomId = null;
if ($start_date && $end_date) {
    $assignedRoomId = getRandomAvailableRoomId($start_date, $end_date, $conn, 'King Suite');
    $_SESSION['assignedRoomId'] = $assignedRoomId; // Store in session
    $_SESSION['start_date'] = $start_date;
    $_SESSION['end_date'] = $end_date;
}
...
*/
########################################################### GPT MED GAMMEL FUNKSJONALITET + NY DB-KOBLING ######################################################################################################################
/*
...
// src/forms/form_kingSuiteBooking.php
...
// Hent tilgjengelige King Suites for de angitte datoene
$availableRooms = $roomModel->getAvailableRoomsByType('King Suite', $checkInDate, $checkOutDate);

if (empty($availableRooms)) {
    echo "<p>Ingen ledige King Suites for de valgte datoene.</p>";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $bookingModel->createBooking(
            ...
            $selectedRoom['id'],
            'King Suite', // Romtype
            $selectedRoom['floor'] ?? 3, // Etasje (hentet fra romdata eller standard)
            false,    // Nær heis (tilpass etter behov)
            true,     // Med utsikt (tilpass etter behov)
            $checkInDate,
            $checkOutDate
        );
...
<head>
    <meta charset="UTF-8">
    <title>Book King Suite</title>
    <link rel="stylesheet" href="../../public/assets/css/style.css">
</head>
<body>
<div class="container">
    <h1>Book King Suite</h1>
